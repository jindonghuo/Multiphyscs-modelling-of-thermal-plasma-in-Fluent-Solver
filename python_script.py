...
originalfiledir = os.path.join(os.getcwd(), 'udfcode')
meshfile    = os.path.join(originalfiledir, 'thick24mm_ionlayer_1layer_denseMesh_v3.cas')    # thick24mm_ionlayer_1layer_denseMesh_v3.cas     ladder_partition_v7_24mm_gassheath_1layer_lessMesh.cas
udf_c_code1 = 'jacob_rightanode_1layer_v3.c'
udf_c_code2 = 'functionfile1.c'
udf_h_code1 = 'headfile1.h'
ge_table1   = 'AirAbsCoeffP1.txt'
ge_table2   = 'Cu_Avearge_Abs_coeff_P1_model_for_UDF.txt'
ge_table3   = 'pa66_cu_air_table_v5.txt'
journal     = r'TUIjournal.jou'
outputfile  = r'output.dat'
...
computingfolder  ='f' + str(timestr) + NameSuffix
...
time.sleep(0.5)         # leave some time for copy
...
f.write('/define/models/species/heat-of-surface-reactions? yes                       \n')
...
# udm and uds
f.write('/define/user-defined/user-defined-memory 34                                 \n')                                                                                  ############### UDM 
...
# Species
f.write('/define/materials/change-create pa66       mix-gas yes constant 1006.43 yes 18.2 yes 0 yes  194336 no no     \n')   # h=2.6e6 J/kg=5.824e8 J/kgmol. Intentional changed to 0.0 
f.write('/define/materials/change-create pa66-solid mix-gas yes constant 1006.43 yes 18.2 yes 0 yes  0 no no          \n')   # Give a small molecular weight for pa66 species.
f.write('/define/materials/change-create cu         mix-gas yes constant 1006.43 yes 63.5 yes 3.14e8 yes 194336 no no \n')   # h=4.901e6 J/kg=3.137e8 J/kgmol
f.write('/define/materials/change-create cu-solid   mix-gas yes constant 1006.43 yes 63.5 yes 0 yes  0  no no         \n')
f.write('/define/materials/change-create air        mix-gas yes constant 1006.43 no no no no no                       \n')
f.write('                      \n')
...
# cu_mass_rate
f.write('/solve/report-definitions/add cu_mrate surface-integral                         \n')
f.write('average-over 1               \n')  #The value reported is averaged over the last 1 iterations/time steps  
f.write('per-surface? no              \n')
f.write('surface-ids wall-anode-anode_drop wall-cathode-cathode_drop-shadow ()           \n')                                          #----------------------Geometry
f.write('field udm-31                 \n')
f.write('quit                         \n')
f.write('                             \n')
...
f.write('active? yes                             \n')
f.write('file-name "'+ os.path.join(casdat_path, 'species_cu.txt') + '" \n')  
f.write('report-defs flow-time cu_mrate ()       \n')         # unit is kg/s at different time
f.write('itr-index "1"                           \n')
f.write('frequency 1                             \n')
f.write('frequency-of time-step                  \n')         # time-step iteration
f.write('run-index "0"                           \n')
...
f.write('uds-0-scalar udm-7 udm-11 udm-13 temperature quit no "rescgns" "time-step" 900 time-step \n') # do not need to specify the zone or surface names.
f.write('             \n')
...
f.write('/file/transient-export/ascii '+os.path.join(casdat_path, 'XYZ_Temp_J ') + '() udm-7 temperature quit yes no "export_asc" "time-step" 900 time-step  \n') # () surfaces 1; "time-step" 
f.write('             \n')
# File--save picuture.
####################################################################################################################################################
f.write('/display/set-window 7                          \n')
f.write('/display/set/picture/landscape yes             \n')  
f.write('/display/set/picture/x-resolution 4000         \n')
...
f.write('/display/save-picture '+ os.path.join(fig_path, 'cu-mole_%t.png')      +      '\n')
f.write('            \n')
...
#
f.write('/solve/execute-commands/add-edit command-3  1 "time-step" "/display/contour udm-7 0 1e5"           \n')
f.write('/solve/execute-commands/add-edit command-4  1 "time-step" "/display/save-picture  '+ os.path.join(fig_path, 'durrD_%t.png') + '"  \n')
f.write('/solve/execute-commands/add-edit command-5  1 "time-step" "/display/contour udm-7 0 3e5"            \n')  # _ _ is auto_range.
f.write('/solve/execute-commands/add-edit command-6  1 "time-step" "/display/save-picture  '+ os.path.join(fig_path, 'eurrD_%t.png') + '"  \n')
f.write('/solve/execute-commands/add-edit command-7  1 "time-step" "/display/contour udm-7 0 5e5"              \n')
f.write('/solve/execute-commands/add-edit command-8  1 "time-step" "/display/save-picture  '+ os.path.join(fig_path, 'furrD_%t.png') + '"  \n')
f.write('/solve/execute-commands/add-edit command-9  1 "time-step" "/display/contour udm-7 0 8e5"              \n')
f.write('/solve/execute-commands/add-edit command-10 1 "time-step" "/display/save-picture  '+ os.path.join(fig_path, 'gurrD_%t.png') + '"  \n')
f.write('/solve/execute-commands/add-edit command-11 1 "time-step" "/display/contour udm-7 0 1.1e6"              \n')
f.write('/solve/execute-commands/add-edit command-12 1 "time-step" "/display/save-picture  '+ os.path.join(fig_path, 'hurrD_%t.png') + '"  \n')

f.write('            \n')

#                                                          ###################################################################### 1st
f.write('/solve/set/time-step 2e-5                  \n')  
f.write('/solve/dual-time-iterate 4 20              \n')  
# all eqations
f.write('/solve/set/equations/uds-0 yes             \n')
f.write('/solve/set/equations/uds-1 yes             \n')
f.write('/solve/set/equations/uds-2 yes             \n')
...
f.write('/solve/set/time-step 2.0e-5                         \n')
f.write('/solve/dual-time-iterate 20 42                      \n')
f.write('/solve/set/time-step 4.0e-5                         \n')
f.write('/solve/dual-time-iterate 20 30                      \n')
f.write('/solve/set/time-step 8.0e-5                         \n')
f.write('/solve/dual-time-iterate 20 30                      \n')

#f.write('/file/transient-export/edit rescgns CGNS '+os.path.join(casdat_path, 'jacres1 '))
#f.write('uds-0-scalar udm-7 udm-11 udm-13 udm-16 udm-29 temperature quit no "time-step" 5 time-step \n') # do not need to specify the zone or surface names.

f.write('/solve/set/time-step 1.2e-4                         \n')
f.write('/solve/dual-time-iterate 20 30                      \n')
f.write('/solve/set/time-step 1.8e-4                         \n')
f.write('/solve/dual-time-iterate 20 30                      \n')
f.write('/solve/set/time-step 2.2e-4                         \n')
f.write('/solve/dual-time-iterate 2800 30                    \n')
...
os.remove(ge_table2) 
os.remove(ge_table3)
os.remove(udf_c_code2)             FigDense3_2_T2000_I0.35_J1e3_0.8e4_2.6e4_V2412_2.22.5
